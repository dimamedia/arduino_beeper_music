...
void setup() {
  ...
  if(debug) Serial.begin(115200); // uncomment for debugging
}
...
void runNote() {
  if((millis() - timer) > noteLength) {
    if(addAfterNotePause) { // after note pause
      ...
      timer = millis();
      if(debug) Serial.print(currentNote); Serial.print(": note "); Serial.print(melody[currentNote]); Serial.print(", bpm: "); Serial.print(BPM); Serial.print(", length: "); Serial.print(noteLength); Serial.print(", timer: "); Serial.println(timer);
